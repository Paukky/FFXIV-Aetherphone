...
- Notification pings: `chat.ping`, `velvet.ping`, `gram.ping`, `social.ping`, `muster.ping`, `announce.ping`, `keys.stale`, and `content.removed`, published onto `RealtimeSignalBus`. `chat.ping` carries the conversation id and, when the server includes it, the pushed message itself.
